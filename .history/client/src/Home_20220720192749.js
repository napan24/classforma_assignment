...
const Home = () => {
    ...
    function getText() {
        placeName.forEach(function (arrayItem) {
            changeColor(document.getElementById('textField'), arrayItem); 
        });
    }
    const changeColor = (a, b) => a.innerHTML = a.innerHTML.replace(b, `<span class="colorName">${b}</span>`);
    const changeColor = (a, b) => a.innerHTML = a.innerHTML.replace(b, `<span class="colorPlace">${b}</span>`);
