...
const Home = () => {
    ...
    const spans = [
        { start: 0, end: length, type: 'person' },
        { start: 25, end: length, type: 'organization' }
    ]
    const ents = [
        { type: 'person', color: { r: 166, g: 226, b: 45 } },
        { type: 'organization', color: { r: 67, g: 198, b: 252 } },
    ]
    useEffect(() => {
        if(textField){
            setLength(textField.length);
        }
    }, [textField]);
    ...
    return (
        <>
            ...
            <Taggy text={textField} spans={spans} ents={ents} />
        </>
    )
}
